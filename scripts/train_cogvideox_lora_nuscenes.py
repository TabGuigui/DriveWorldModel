"""LoRA fine-tuning for CogVideoX on DriveWorldModel manifests.

This script follows the official diffusers CogVideoX LoRA trainer structure,
but replaces the prompt/video text files with the normalized DriveWorldModel
jsonl manifest used by the nuScenes adapter.
"""
...
def train(config: ExperimentConfig, cli_args: argparse.Namespace) -> None:
    import numpy as np
    import torch
    ...
    accelerator = Accelerator(
        ...
        log_with=cli_args.report_to or train_extra.get("report_to"),
    ...
    params_to_optimize = [{"params": [p for p in transformer.parameters() if p.requires_grad], "lr": training.learning_rate}]
    ...
    train_dataset = NuScenesCogVideoXDataset(
        config=config,
        torch=torch,
        np=np,
        transforms=TT,
    ...
    progress_encode_bar = tqdm(range(num_videos), desc="Encoding videos", disable=not accelerator.is_local_main_process)
    ...
    num_trainable_parameters = sum(p.numel() for group in params_to_optimize for p in group["params"])
    ...
    resume_from_checkpoint = cli_args.resume_from_checkpoint or train_extra.get("resume_from_checkpoint")
        ...
        for batch in train_dataloader:
            ...
            if accelerator.sync_gradients:
                ...
                if global_step % training.checkpointing_steps == 0:
                    save_checkpoint(accelerator, output_dir, global_step, int(train_extra.get("checkpoints_total_limit", 0)))
    ...
    if accelerator.is_main_process:
        ...
        dtype = torch.float16 if training.mixed_precision == "fp16" else torch.bfloat16 if training.mixed_precision == "bf16" else torch.float32
        ...
        validation_prompt = cli_args.validation_prompt or train_extra.get("validation_prompt")
        if validation_prompt and cli_args.num_validation_videos > 0:
            del transformer
            free_memory()
            pipe = CogVideoXPipeline.from_pretrained(pretrained, revision=revision, variant=variant, torch_dtype=weight_dtype)
            ...
            for i in range(cli_args.num_validation_videos):
                frames = pipe(
                ...
                export_to_video(frames, str(output_dir / f"validation_{i}.mp4"), fps=generation.fps)

    accelerator.end_training()


class NuScenesCogVideoXDataset:
    def __init__(
        self,
        ...
        self.records = load_manifest_records(config)
...
def load_manifest_records(config: ExperimentConfig) -> list[ManifestRecord]:
    manifest_path = Path(config.dataset.manifest_path) if config.dataset.manifest_path else Path(config.dataset.root) / "manifests" / f"{config.dataset.split}.jsonl"
    ...
    with manifest_path.open("r", encoding="utf-8") as handle:
        for line_number, line in enumerate(handle, start=1):
            ...
            if not line:
                continue
            raw = json.loads(line)
            target_video = raw.get(config.training.target_video_key) or raw.get("target_video") or raw.get("video")
            if target_video is None:
                raise KeyError(f"{manifest_path}:{line_number} missing target_video/video field")
            target_path = Path(target_video)
            if not target_path.is_absolute():
                target_path = Path(config.dataset.root) / target_path
            records.append(
                ManifestRecord(
                    sample_id=str(raw.get("sample_id", raw.get("sample_token", line_number))),
                    prompt=config.generation.prompt or raw.get("prompt", ""),
...
def compute_prompt_embeddings(tokenizer, text_encoder, prompt, max_sequence_length, device, dtype, t5_tokenizer_cls):
...
def flatten_config(config: ExperimentConfig) -> dict[str, Any]:
    return {
        "name": config.name,
